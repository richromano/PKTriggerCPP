...
int save_buffer(pslr_handle_t camhandle, int bufno, int fd, pslr_status *status, user_file_format filefmt, int jpeg_stars) {
    ...
    while (true) {
        ...
        uint8_t fixbuf[88] = { 0x4d, 0x4d, 0x00, 0x2a, 0x00, 0x00, 0x00, 0x08,
        ...
        if ((current == 0)&&(filefmt == USER_FILE_FORMAT_PEF))
            for (int i=0; i < 88; i++)
                buf[i] = fixbuf[i];

        int r = _write(fd, buf, bytes);
        if (r == 0) {
...
int main(int argc, char **argv) {
    ...
    pslr_color_space_t color_space;
    pslr_af_mode_t af_mode;
    pslr_ae_metering_t ae_metering;
    pslr_flash_mode_t flash_mode;
    pslr_drive_mode_t drive_mode;
    pslr_af_point_sel_t af_point_sel;
    uint32_t af_point_selected = 0;
    pslr_jpeg_image_tone_t jpeg_image_tone;
	pslr_white_balance_mode_t white_balance_mode;
    ...
    if ( dump_memory_size > 0 ) {
        int dfd = _open(DUMP_FILE_NAME, FILE_ACCESS, 0664);
        if (dfd == -1) {
            pslr_write_log(PSLR_ERROR, "Could not open %s\n", DUMP_FILE_NAME);
            return -1;
        } else {
    ...
    if ( modify_debug_mode == 1) {
        pslr_set_debugmode((ipslr_handle_t *)camhandle,debug_mode);
        pslr_camera_close(camhandle);
        exit(0);
    }
