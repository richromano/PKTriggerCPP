...
PKTRIGGERCORDDLL_API int gettimeofday(struct timeval * tp, void * tzp)
{
...
PKTRIGGERCORDDLL_API void usleep(__int64 usec)
{
	HANDLE timer;
	LARGE_INTEGER ft;
...
PKTRIGGERCORDDLL_API void hexdump(uint8_t *buf, uint32_t bufLen);
...
PKTRIGGERCORDDLL_API user_file_format_t *pslr_get_user_file_format_t( user_file_format uff ) {
    unsigned int i;
...
PKTRIGGERCORDDLL_API int pslr_set_user_file_format(pslr_handle_t h, user_file_format uff) {
    switch ( uff ) {
...
PKTRIGGERCORDDLL_API user_file_format pslr_get_user_file_format( pslr_status *st ) {
...
PKTRIGGERCORDDLL_API pslr_handle_t pslr_init( char *model, char *device ) {
    ...
    if ( device == NULL ) {
        ...
        driveNum = 1;
        drives = (char**)malloc( driveNum * sizeof(char*) );
        drives[0] = _strdup( device );
    }
    DPRINT("driveNum:%d\n",driveNum);
    int i;
...
PKTRIGGERCORDDLL_API char *pslr_get_status_info( pslr_handle_t h, pslr_status status ) {
    ...
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "jpeg image tone", pslr_get_jpeg_image_tone_str((pslr_jpeg_image_tone_t)status.jpeg_image_tone));
    ...
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %d\n", "focus", status.focus);
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "color space", pslr_get_color_space_str((pslr_color_space_t)status.color_space));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "image format", pslr_get_image_format_str((pslr_image_format_t)status.image_format));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "raw format", pslr_get_raw_format_str((pslr_raw_format_t)status.raw_format));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %d\n", "light meter flags", status.light_meter_flags);
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "ec", pslr_format_rational( status.ec, "%.2f" ) );
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s EV steps\n", "custom ev steps", pslr_get_custom_ev_steps_str((pslr_custom_ev_steps_t)status.custom_ev_steps));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s EV steps\n", "custom sensitivity steps", pslr_get_custom_sensitivity_steps_str((pslr_custom_sensitivity_steps_t)status.custom_sensitivity_steps));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %d\n", "exposure mode", status.exposure_mode);
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "scene mode", pslr_get_scene_mode_str((pslr_scene_mode_t)status.scene_mode));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %d\n", "user mode flag", status.user_mode_flag);
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "ae metering mode", pslr_get_ae_metering_str((pslr_ae_metering_t)status.ae_metering_mode));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "af mode", pslr_get_af_mode_str((pslr_af_mode_t)status.af_mode));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "af point select", pslr_get_af_point_sel_str((pslr_af_point_sel_t)status.af_point_select));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "selected af point", pslr_get_af_name( h, status.selected_af_point));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "focused af point", pslr_get_af_name( h, status.focused_af_point));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "drive mode", pslr_get_drive_mode_str((pslr_drive_mode_t)status.drive_mode));
    ...
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "white balance mode", pslr_get_white_balance_mode_str((pslr_white_balance_mode_t)status.white_balance_mode));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "white balance adjust", get_white_balance_adjust_str(status.white_balance_adjust_mg, status.white_balance_adjust_ba));
    sprintf(strbuffer+strlen(strbuffer),"%-32s: %s\n", "flash mode", pslr_get_flash_mode_str((pslr_flash_mode_t)status.flash_mode));
...
char *get_hardwired_setting_bool_info( pslr_bool_setting setting) {
    char *strbuffer = (char*)malloc(32+1);
...
PKTRIGGERCORDDLL_API char *pslr_get_settings_info( pslr_handle_t h, pslr_settings settings ) {
    char *strbuffer = (char*)malloc(8192);
...
PKTRIGGERCORDDLL_API int pslr_set_shutter(pslr_handle_t h, pslr_rational_t value) {
...
PKTRIGGERCORDDLL_API int pslr_set_aperture(pslr_handle_t h, pslr_rational_t value) {
    DPRINT("[C]\tpslr_set_aperture(%x %x)\n", value.nom, value.denom);
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return ipslr_handle_command_x18( p, false, X18_APERTURE, 3, value.nom, value.denom, 0);
}

PKTRIGGERCORDDLL_API int pslr_set_iso(pslr_handle_t h, uint32_t value, uint32_t auto_min_value, uint32_t auto_max_value) {
...
PKTRIGGERCORDDLL_API int pslr_set_expose_compensation(pslr_handle_t h, pslr_rational_t value) {
    DPRINT("[C]\tpslr_set_expose_compensation(0x%X 0x%X)\n", value.nom, value.denom);
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return ipslr_handle_command_x18( p, true, X18_EC, 2, value.nom, value.denom, 0);
}

PKTRIGGERCORDDLL_API int pslr_set_white_balance(pslr_handle_t h, pslr_white_balance_mode_t wb_mode) {
    DPRINT("[C]\tpslr_set_white_balance(0x%X)\n", wb_mode);
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return ipslr_handle_command_x18( p, true, X18_WHITE_BALANCE, 1, wb_mode);
}

PKTRIGGERCORDDLL_API int pslr_set_white_balance_adjustment(pslr_handle_t h, pslr_white_balance_mode_t wb_mode, uint32_t wbadj_mg, uint32_t wbadj_ba) {
...
PKTRIGGERCORDDLL_API int pslr_set_flash_mode(pslr_handle_t h, pslr_flash_mode_t value) {
    DPRINT("[C]\tpslr_set_flash_mode(%X)\n", value);
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return ipslr_handle_command_x18( p, true, X18_FLASH_MODE, 1, value, 0, 0);
}

PKTRIGGERCORDDLL_API int pslr_set_flash_exposure_compensation(pslr_handle_t h, pslr_rational_t value) {
...
PKTRIGGERCORDDLL_API int pslr_set_drive_mode(pslr_handle_t h, pslr_drive_mode_t drive_mode) {
    DPRINT("[C]\tpslr_set_drive_mode(%X)\n", drive_mode);
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return ipslr_handle_command_x18( p, true, X18_DRIVE_MODE, 1, drive_mode, 0, 0);
}

PKTRIGGERCORDDLL_API int pslr_set_ae_metering_mode(pslr_handle_t h, pslr_ae_metering_t ae_metering_mode) {
    DPRINT("[C]\tpslr_set_ae_metering_mode(%X)\n", ae_metering_mode);
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return ipslr_handle_command_x18( p, true, X18_AE_METERING_MODE, 1, ae_metering_mode, 0, 0);
}

PKTRIGGERCORDDLL_API int pslr_set_af_mode(pslr_handle_t h, pslr_af_mode_t af_mode) {
    DPRINT("[C]\tpslr_set_af_mode(%X)\n", af_mode);
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return ipslr_handle_command_x18( p, true, X18_AF_MODE, 1, af_mode, 0, 0);
}

PKTRIGGERCORDDLL_API int pslr_set_af_point_sel(pslr_handle_t h, pslr_af_point_sel_t af_point_sel) {
    DPRINT("[C]\tpslr_set_af_point_sel(%X)\n", af_point_sel);
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return ipslr_handle_command_x18( p, true, X18_AF_POINT_SEL, 1, af_point_sel, 0, 0);
}

PKTRIGGERCORDDLL_API int pslr_set_jpeg_stars(pslr_handle_t h, int jpeg_stars ) {
...
PKTRIGGERCORDDLL_API int pslr_set_jpeg_resolution(pslr_handle_t h, int megapixel) {
...
PKTRIGGERCORDDLL_API int pslr_set_jpeg_image_tone(pslr_handle_t h, pslr_jpeg_image_tone_t image_tone) {
...
PKTRIGGERCORDDLL_API int pslr_set_color_space(pslr_handle_t h, pslr_color_space_t color_space) {
    DPRINT("[C]\tpslr_set_raw_format(%X)\n", color_space);
    ipslr_handle_t *p = (ipslr_handle_t *) h;
...
PKTRIGGERCORDDLL_API int pslr_delete_buffer(pslr_handle_t h, int bufno) {
...
PKTRIGGERCORDDLL_API int pslr_set_exposure_mode(pslr_handle_t h, pslr_exposure_mode_t mode) {
...
PKTRIGGERCORDDLL_API int pslr_buffer_open(pslr_handle_t h, int bufno, pslr_buffer_type buftype, int bufres) {
...
PKTRIGGERCORDDLL_API uint32_t pslr_fullmemory_read(pslr_handle_t h, uint8_t *buf, uint32_t offset, uint32_t size) {
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    int ret;
...
PKTRIGGERCORDDLL_API uint32_t pslr_buffer_get_size(pslr_handle_t h) {
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    uint32_t i;
    uint32_t len = 0;
    for (i = 0; i < p->segment_count; i++) {
...
PKTRIGGERCORDDLL_API void pslr_buffer_close(pslr_handle_t h) {
    ipslr_handle_t *p = (ipslr_handle_t *) h;
...
PKTRIGGERCORDDLL_API int pslr_set_selected_af_point(pslr_handle_t h, uint32_t point) {
    ipslr_handle_t *p = (ipslr_handle_t *) h;
...
PKTRIGGERCORDDLL_API bool pslr_get_model_only_limited(pslr_handle_t h) {
    ipslr_handle_t *p = (ipslr_handle_t *) h;
...
PKTRIGGERCORDDLL_API  pslr_jpeg_image_tone_t pslr_get_model_max_supported_image_tone(pslr_handle_t h) {
    ipslr_handle_t *p = (ipslr_handle_t *) h;
...
PKTRIGGERCORDDLL_API bool pslr_get_model_old_bulb_mode(pslr_handle_t h) {
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return p->model->old_bulb_mode;
}

PKTRIGGERCORDDLL_API bool pslr_get_model_bufmask_single(pslr_handle_t h) {
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return p->model->bufmask_single;
}

PKTRIGGERCORDDLL_API bool pslr_get_model_has_settings_parser(pslr_handle_t h) {
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    char cameraid[10];
...
PKTRIGGERCORDDLL_API const char *pslr_get_camera_name(pslr_handle_t h) {
...
PKTRIGGERCORDDLL_API pslr_buffer_type pslr_get_jpeg_buffer_type(pslr_handle_t h, int jpeg_stars) {
    ipslr_handle_t *p = (ipslr_handle_t *) h;
    return (pslr_buffer_type)(2 + pslr_get_hw_jpeg_quality( p->model, jpeg_stars ));
}
...
static int ipslr_status_full(ipslr_handle_t *p, pslr_status *status) {
    ...
    if ( expected_bufsize == 0 || !p->model->status_parser_function ) {
        ...
        if ( p->model->need_exposure_mode_conversion ) {
            status->exposure_mode = exposure_mode_conversion( (pslr_exposure_mode_t)status->exposure_mode );
        }
...
static int _ipslr_write_args(uint8_t cmd_2, ipslr_handle_t *p, int n, ...) {
    ...
    uint8_t *buf=(uint8_t*)malloc(4 * n);
    FDTYPE fd = p->fd;
    int res;
    int i;
    uint32_t data;
